...
static void bss_init(void) {
  ...
  /* Fix: Use proper pointer arithmetic with size_t */
  size_t bss_size = (size_t)(&_end_bss - &_start_bss);
  memset(&_start_bss, 0, bss_size);
}
...
static void paging_init(uint64_t mem_end) {
  ...
  for (uint64_t pa = 0; pa < mem_end; pa += PGSIZE) {
    ...
    int perm = PTE_P | PTE_W; /* Fix: Move perm declaration inside loop */
    
    if ((uint64_t)&start <= va && va < (uint64_t)&_end_kernel_text)
      perm &= ~PTE_W;

    uint64_t *pte; /* Fix: Move pte declaration inside loop */
...
static char **read_command_line(void) {
  ...
  char *p;
  const char *end; /* Fix: Make end const */
  int argc;
  int i;
...
static void power_off(void) { /* Fix: Make power_off static */
#ifdef FILESYS
  filesys_done();
#endif
